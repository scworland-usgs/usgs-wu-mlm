...
wudata2 <- wudata %>%
  dplyr::select(c(wn,census_region,climate_region,CDC_urban,year)) %>%
...
# combine estimates
anova.combined <- data.frame (rbind(anova.census, anova.climate, anova.urban, anova.year)) %>%
...
ggplot(filter(anova.combined,!grepl("within",Source))) + ylab(expression(sigma / df)) +
  ...
  scale_x_discrete(labels=c("census region","climate region","urban class","years"))
